    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            signed_file = ContentFile(
                signed_content, name=f"signed_{document_file.name}"
            )
            ...
            messages.error(
                request, "Hubo un error con el formulario. Intenta de nuevo."
            )
    else:
...
def view_signed_documents(request):
    ...
    return render(
        request,
        "accounts/view_signed_documents.html",
    ...
    if signed_document.document_file:
        try:
            ...
            response = FileResponse(
                signed_document.document_file.open("rb"), as_attachment=True
            )
            response["Content-Disposition"] = (
                f'attachment; filename="{signed_document.document_file.name}"'
            )
            return response
    ...
    if request.method == "POST" and "document_file" in request.FILES:
        ...
        try:
            ...
            if file_content.count(b"\n---SIGNATURE---\n") != 1:
                messages.error(
                    request,
                    "El archivo no contiene un formato válido de firma digital.",
                )
            ...
            profile = (
                user.userprofile
            )  # Asegúrate de que UserProfile esté relacionado con User
            ...
            try:
                ...
                messages.success(
                    request,
                    "El documento se verificó correctamente. La firma es válida.",
                )
            except (ValueError, TypeError):
                messages.error(
                    request, "La firma no es válida o el documento ha sido alterado."
                )
        except Exception as e:
...
def verificar_firma(request):
    ...
    if request.method == "POST" and "document_file" in request.FILES:
        ...
        try:
            ...
            for firma_registrada in firmas_registradas:
                ...
                try:
                    contenido_almacenado, firma_almacenada = firmado_data.split(
                        b"\n---SIGNATURE---\n"
                    )
                except ValueError:
                ...
                try:
                    ...
                    llave_publica = RSA.import_key(
                        user_profile.public_key.encode("utf-8")
                    )
                except UserProfile.DoesNotExist:
    ...
    return render(
        request,
        "accounts/verificar_firma.html",
        {"datos_firma": datos_firma, "error": error},
    )
